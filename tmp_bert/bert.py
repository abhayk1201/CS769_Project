...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        # query, key, value has shape [bs, num_attention_heads, seq_len, attention_head_size(64)]
        bs, n_heads, seq_len = query.shape[:3]
        ...
        attn_scores = F.softmax(attn_scores, dim=-1)  # [bs, num_attention_heads, seq_len, seq_len]
        # attn_scores = self.dropout(attn_scores)  # NOT_SURE if dropout is needed.

        # multiply the attention scores to the value and get back V'
        attn_value = torch.matmul(attn_scores, value)  # [bs, num_attention_heads, seq_len, attention_head_size(64)]
        ...
        attn_value = attn_value.transpose(1, 2).contiguous().view(bs, seq_len, -1)
        return attn_value
...
class BertLayer(nn.Module):
    ...
    """
        ...
        # attn_heads = [bs, seq_len, num_attention_heads * attention_head_size]  -->  [bs, seq_len, hidden_size]
        attn_heads = self.self_attention(hidden_states, attention_mask)

        # add-norm layer: attn_add_norm --> [bs, seq_len, hidden_size]
        attn_add_norm = self.add_norm(hidden_states, attn_heads, self.attention_dense, self.attention_dropout,
                                      self.attention_layer_norm)

        # feed forward
        interm_hidden = self.interm_af(self.interm_dense(attn_add_norm))

        # another add-norm layer
        output = self.add_norm(attn_add_norm, interm_hidden, self.out_dense, self.out_dropout,
                               self.out_layer_norm)
        return output
...
class BertModel(BertPreTrainedModel):
  ...
  """
    ...
    def embed(self, input_ids):
        ...
        pos_embeds = self.pos_embedding(pos_ids)  # [1 x seq_len x positional_encoding_dim] --> broadcast later
