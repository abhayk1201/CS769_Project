...
class BertSentClassifier(torch.nn.Module):
    def __init__(self, config):
        ...
        self.linear_layer = torch.nn.Linear(config.hidden_size, config.num_labels)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        logits = self.linear_layer(self.dropout(pooled_output))
...
class BertDataset(Dataset):
    ...
    def collate_fn(self, all_data):  # create training batch data
...
def train(args):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=3, factor=args.lr_schedular_factor, verbose=True)
    ...
    for epoch in range(args.epochs):
        ...
        epoch_dev_accuracy = dev_acc
        if epoch_dev_accuracy < last_dev_accuracy:
            trigger_times += 1
            print("NO INCREASE in dev accuracy")
            if trigger_times >= patience:
                print('*' * 50)
                print(f'\n\n Early stopping as dev accuracy has not increased for last {patience} epochs')
                break
        else:
            trigger_times = 0
        ...
        # Learning rate schedular
        scheduler.step(epoch_dev_accuracy)
...
def get_args():
    ...
    parser.add_argument("--lr", type=float, help="learning rate, default lr for 'pretrain': 1e-3, 'finetune': 1e-5",
                        default=1e-5)
    parser.add_argument("--lr_schedular_factor", type=float, help="learning rate schedular factor", default=0.9)
